...
class CompactBilinearPoolingFn(Function):
    ...
    def forward(ctx, h1, s1, h2, s2, output_size, x, y, force_cpu_scatter_add=False):
        ...
        fx = torch.fft.rfft(px, dim=1, norm="ortho")
        # re_fx = fx.select(-1, 0)
        re_fx = fx.real
        # im_fx = fx.select(-1, 1)
        im_fx = fx.imag
        ...
        fy = torch.fft.rfft(py, dim=1, norm="ortho")
        # re_fy = fy.select(-1, 0)
        re_fy = fy.real
        # im_fy = fy.select(-1, 1)
        im_fy = fy.imag
        ...
        compx = torch.complex(re_prod, im_prod)
        re = torch.fft.irfft(compx, dim=1, norm="ortho")
    ...
    def backward(ctx, grad_output):
        ...
        grad_fx = torch.fft.irfft(torch.stack((grad_re_fx, grad_im_fx), grad_re_fx.dim()), 1,
        ...
        grad_fy = torch.fft.irfft(torch.stack((grad_re_fy, grad_im_fy), grad_re_fy.dim()), 1,
                              signal_sizes=(ctx.output_size,))
...
class CompactBilinearPooling(nn.Module):
    ...
    def __init__(self, input1_size, input2_size, output_size, h1=None, s1=None, h2=None, s2=None,
                 force_cpu_scatter_add=False, **kwargs):
